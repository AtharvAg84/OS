...
void firstFit(int partitions[], int m, int processes[], int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Search for the first fit partition for the process
        for (int j = 0; j < m; j++) {
            // If the partition is large enough, allocate the process
            if (partitions[j] >= processes[i]) {
                allocation[i] = j;
                partitions[j] -= processes[i];  // Update partition size after allocation
                allocated = 1;
                break;
            }
        }

        // If no partition is found, the process remains unallocated
        if (allocated == -1) {
            allocation[i] = -1;
        }
    }
